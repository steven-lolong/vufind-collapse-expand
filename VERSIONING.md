...
## Basic considerations related to VuFind-Versioning
Since this module is only usable together with VuFind, we need to make it clear with which VuFind versions the current module version will be compatible.
...
**=> Conclusion 1)** It might cause less confusion in the end, if our own versions just start with the VuFind version and add our own versioning at the end.
...
**=> Conclusion 2)** Our versioning schema should contain at least VuFind version X.Y + our own versioning.
...
**Option 1) X.Y.Z.A.B.C** => 10.2.0.1.0.0 => compatible with VuFind 10.2.0 in our own version 1.0.0
...
**Option 2) X.Y.A.B** => 10.2.1.0 => compatible with VuFind 10.2 in our own version 1.0
...
Sequence: X.Y.A.B => A.B-X.Y Should we move the VuFind Version to the end, e.g. 1.0-10.2? However this might cause expectations for the same module version to have exactly the same features across all VuFind-versions, even if some features might change due to altered Solr/VuFind versions.
